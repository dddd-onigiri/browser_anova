...
st.set_page_config(page_title="ブラウザ一元配置分散分析(対応なし)", layout="wide")

st.title("ブラウザ一元配置分散分析(対応なし)")
st.caption("Created by Daiki Ito")
st.write("")
st.subheader("ブラウザで一元配置分散分析　→　表　→　解釈まで出力できるウェブアプリです。")
...
with st.form(key='variable_form'):
    ...
    if ivType != 'object':
        ...
        st.write(
            ...
            unsafe_allow_html=True)
        st.write(
            '<span style="color:red">　例１）A組・B組・C組</span>',
            unsafe_allow_html=True)
        st.write(
            '<span style="color:red">　例２）１年・２年・３年</span>',
            unsafe_allow_html=True)
    else:
        st.write(
...
with st.form(key='check_form'):
    ...
    # 分析実行ボタンの表示
    ANOVA_btn = st.form_submit_button('分析実行')
